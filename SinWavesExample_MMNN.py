...
from FunctionEncoder import SinWavesDataset, FunctionEncoder, MSECallback, ListCallback, TensorboardCallback, \
    DistanceCallback
...
if load_path is None:
    logdir = f"logs/1DSinWaves_example/{train_method}/{arch}/{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
else:
    logdir = load_path
...
if load_path is None:
    ...
    if arch == "MLP":
        ...
        for param_group in model.opt.param_groups:
            param_group['lr'] = lr
        print("learning rate set at ", param_group['lr'])
    else:
        model = FunctionEncoder(input_size=dataset.input_size,
                                output_size=dataset.output_size,
                                data_type=dataset.data_type,
...
with torch.no_grad():
    ...
    for i in range(n_plots):
        ...
        if i == n_plots - 1:
            ax.legend()
        title = f"a = {info['As'][i].item():.2f}, b = {info['Bs'][i].item():.2f}, c = {info['Cs'][i].item():.2f}"
        ax.set_title(title)
    ...
    query_xs = torch.linspace(dataset.input_range[0], dataset.input_range[1], 1000).reshape(1000, 1).to(device)
