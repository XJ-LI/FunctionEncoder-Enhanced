...
class SinWavesDataset(BaseDataset):
    def __init__(self,
                 a_range=(-1, 1),
                 b_range=(-1, 1),
                 c_range=(-1, 1),
    ...
    def sample(self) -> Tuple[  torch.tensor, 
        ...
        with torch.no_grad():
            ...
            query_ys = torch.cos(As.unsqueeze(1) * pi * query_xs) - torch.sin(Bs.unsqueeze(1) * pi * query_xs**2)*torch.cos(Cs.unsqueeze(1) * pi * query_xs**3)
            example_ys = torch.cos(As.unsqueeze(1) * pi * example_xs) - torch.sin(Bs.unsqueeze(1) * pi * example_xs**2)*torch.cos(Cs.unsqueeze(1) * pi * example_xs**3)
