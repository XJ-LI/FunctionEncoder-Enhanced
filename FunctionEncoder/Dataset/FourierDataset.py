...
class FourierDataset(BaseDataset):
    ...
    def sample(self) -> Tuple[  torch.tensor, 
        ...
        with torch.no_grad():
            ...
            query_ys = As.unsqueeze(1) * torch.sin(self.scale_factor * pi*query_xs) + Bs.unsqueeze(1) * torch.sin(self.scale_factor * 3*pi*query_xs) + Cs.unsqueeze(1) * torch.sin(self.scale_factor * 5*pi*query_xs)
            example_ys = As.unsqueeze(1) * torch.sin(self.scale_factor * pi*example_xs) + Bs.unsqueeze(1) * torch.sin(self.scale_factor * 3*pi*example_xs) + Cs.unsqueeze(1) * torch.sin(self.scale_factor * 5*pi*example_xs)
